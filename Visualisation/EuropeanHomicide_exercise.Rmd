...
author: "Vladimir Bountovski and Oscar Robert Nixon"
...
ggsave("homicides5.png" , width = 15, height = 8, dpi = 150)
```
...
```{r eval = TRUE}
kings <- read.csv("data/Danskekonger.csv", sep=";")

kings$midyear <- kings$end_year - (kings$end_year - kings$start_year)/2

ggplot(kings, aes(x = midyear, y = reign_years)) +
  geom_smooth() +
  labs(
    title = "Danish monarchs over time",
    x = "Year",
    y = "Reign length"
  )
```
...
The data shows us that the homicide rates in Western Europe have declined significantly over the long term. In earlier centuries, the homicide rates were much higher. In some cases exceeding 50 per 100,000 people whereas today are much lower. This suggests that the societies have become more peaceful and therefor, possibly ore "civilized" over time. One important reason could be the development of stronger states and legal systems. Governments became better at enforcing laws and maintaning order, individuals were less likely to resolve conflicts through violence. Furthermore, social norms have changed. Violence is no longer seen as acceptable way to handle disputes, and therefore are people more likely to rely on institutions such as police. However, it is important to know that this does not mean that mordern societies are free from violence. The data only captures homicide rates and does not include any other form of violence, like structural or psychological violence. Overall, this data supports the idea the we are more civilized today, but maybe this conclusion should be intepreted with caution.